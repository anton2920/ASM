...
#define SONG ("/home/anton/Музыка/Kissing Strangers/DNCE  -  Kissing Strangers.wav")
...
int main()
{
  int fd, file;	/* sound device file descriptor */
  int arg;	/* argument for ioctl calls */
  int status;   /* return status of system calls */

  /* open sound device */
  fd = open("/dev/dsp2", O_RDWR);
  if (fd < 0) {
  ...
  if (!(file = open(SONG, 00))) {
      exit(0);
  }

  while (1) { /* loop until Control-C */
    printf("Say something:\n");
    status = read(file, buf, sizeof(buf)); /* record some sound */
